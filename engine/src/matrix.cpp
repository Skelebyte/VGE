...
uint Matrix::TotalEntries() const { return entries; }

uint Matrix::GetColumns() const { return dimensions.x; }

uint Matrix::GetRows() const { return dimensions.y; }
...
float Matrix::GetEntry(uint columnIdx, uint rowIdx) const {
...
void Matrix::SetEntry(uint columnIdx, uint rowIdx, float value) const {
  if (columnIdx > GetColumns() - 1) {
    Logger::LOG("There are only " + ToString(GetColumns()) + " (" +
                ToString(GetColumns() - 1) +
                ") columns in this matrix! Column " + ToString(columnIdx) +
